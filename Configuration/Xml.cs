...
namespace System.NET.CSharp.Configuration
{
	public class Xml : XmlBased
	{
		...
		public Xml(Xml xml) : base(xml) => m_rootName = xml.m_rootName;

		public override string DefaultName => DefaultNameWithoutExtension + ".xml";
		public override object Clone() => new Xml(this);
		private string GetSectionPath(string section) => "section[@name=\"" + section + "\"]";
		...
		public string RootName
		{
			...
			{
				VerifyNotReadOnly();
				if (m_rootName == value.Trim() || RaiseChangeEvent(true, ProfileChangeType.Other, null, null, value))
					return;
				m_rootName = value.Trim();
				RaiseChangeEvent(false, ProfileChangeType.Other, null, nameof(RootName), value);
			}
		}
		...
		public override void SetValue(string section, string entry, object value)
		{
			...
			else
			{
				...
				else
				{
					...
					XmlNode xmlNode1 = documentElement.SelectSingleNode(GetSectionPath(section));
					if (xmlNode1 == null)
					{
					...
					if (xmlNode2 == null)
					{
						...
						element.Attributes.Append(attribute);
						xmlNode2 = xmlNode2.AppendChild(element);
					}
		...
		public override object GetValue(string section, string entry)
		{
			...
			try
			{
				return GetXmlDocument().DocumentElement.SelectSingleNode(GetSectionPath(section) + "/" + GetEntryPath(entry)).InnerText;
			}
			catch
			{
				return null;
		...
		public override void RemoveEntry(string section, string entry)
		{
			...
			if (xmlDocument == null)
				return;
			XmlNode oldChild = xmlDocument.DocumentElement.SelectSingleNode(GetSectionPath(section) + "/" + GetEntryPath(entry));
		...
		public override void RemoveSection(string section)
		{
			...
			if (documentElement == null)
				return;
			XmlNode oldChild = documentElement.SelectSingleNode(GetSectionPath(section));
		...
		public override string[] GetEntryNames(string section)
		{
			if (!HasSection(section))
				return null;
			VerifyAndAdjustSection(ref section);
			XmlNodeList xmlNodeList = GetXmlDocument().DocumentElement.SelectNodes(GetSectionPath(section) + "/entry[@name]");
			if (xmlNodeList == null)
				return null;
